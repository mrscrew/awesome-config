...
function menu:init(args)
    ...
    local separator = args.separator or
                          {widget = redflat.gauge.separator.horizontal()}
    local theme = args.theme or {auto_hotkey = true}
    local icon_style = args.icon_style or
                           {custom_only = true, scalable_only = true}

    -- theme vars
    local default_icon = redflat.util.base.placeholder()
    local icon = redflat.util.table.check(beautiful, "icon.awesome") and
                     beautiful.icon.awesome or default_icon
    local color = redflat.util.table.check(beautiful, "color.icon") and
                      beautiful.color.icon or nil
    ...
    local awesomemenu = {
        {
            ...
            micon("gnome-session-reboot")
        }, separator, {
            "конфигурация Awesome", env.fm .. " .config/awesome",
            micon("folder-bookmarks")
        }, {
            "библиотека Awesome", env.fm .. " /usr/share/awesome/lib",
            micon("folder-bookmarks")
        }
    }
    ...
    local exitmenu = {
        ...
        {"Выключение", "shutdown now", micon("system-shutdown")},
        separator, {
            "Переключить пользователя",
            "dm-tool switch-to-greeter", micon("gnome-session-switch")
        }, {
            "Режим ожидания", "systemctl suspend",
            micon("gnome-session-suspend")
    ...
    self.mainmenu = redflat.menu({
        ...
        items = {
            ...
            separator, {"Терминал", env.terminal, micon("terminal")},
            {
                "Файловый менеджер",
                env.fm,
                micon("folder"),
            ...
            {"Редактор", "emacs", micon("emacs")}, separator,
