...
qlaunch.keys.action = {
    {
        ...
        {
            description = "Закрыть виджет",
            group = "Действие"
        }
    }, {
        {}, "Return", function() qlaunch:run_and_hide() end, {
            description = "Запустить или поднять выбранное приложение",
            group = "Действие"
        }
    }, {
        {}, "s",
        function()
            qlaunch:set_new_app(qlaunch.switcher.selected, client.focus)
        end, {
            description = "Привязать сфокусированное приложение к выбранной клавише",
            group = "Действие"
        }
    }, {
        {}, "d", function()
            qlaunch:set_new_app(qlaunch.switcher.selected)
        end, {
            description = "Очистить выбранный ключ",
            group = "Действие"
        }
    }, {
        {}, "r", function() qlaunch:load_config(true) end, {
            description = "Перезагрузить конфигурацию с диска",
            group = "Действие"
        }
    }, {
        {}, "w", function() qlaunch:save_config() end, {
            description = "Сохранить конфигурацию на диск",
            group = "Действие"
        }
    }, {
...
qlaunch._fake_keys = {
    {
        {}, "N", nil, {
            description = "Выберите приложение (запустить или поднять, если оно уже выбрано) с помощью клавиши",
            group = "Действие",
...
local function default_style()
    ...
    return redutil.table.merge(style, redutil.table
                                   .check(beautiful, "float.qlaunch") or {})
end
...
local function get_clients(app)
    ...
    for _, c in ipairs(client.get()) do
        if c.class and c.class:lower() == app then
...
local function build_filter(app)
    return function(c) return c.class:lower() == app end
end
...
local function build_state_indicator(style)
    ...
    function widg:draw(_, cr, width, height)
        local n = #data.state
        local x0 = (width - n * style.state.size - (n - 1) * style.state.gap) /
                       2
        local y0 = (height - style.state.size) / 2

        for i = 1, n do
            cr:set_source(color(data.state[i].focused and style.color.main or
                                    data.state[i].urgent and style.color.urgent or
                                    data.state[i].minimized and style.color.gray or
                                    style.color.icon))
...
local function build_item(key, style)
    ...
    local label = wibox.widget({
        markup = string.format('<span color="%s">%s</span>', style.color.text,
                               key),
        align = "center",
    ...
    widg.layout:set_middle(wibox.container.margin(icon_align,
                                                  unpack(style.appline.igap)))
...
local function build_switcher(keys, style)
    ...
    for _, key in ipairs(sk) do
        ...
        middle_layout:add(wibox.container.margin(widg.items[key].layout,
                                                 unpack(style.appline.im)))
    end
    ...
    function widg:update(store, idb)
        ...
        for key, data in pairs(store) do
            local icon = data.app == "" and style.no_icon or idb[data.app] or
                             style.df_icon
    ...
    function widg:reset()
        for _, item in pairs(self.items) do
            item.background:set_bg(style.color.bg)
        end
        self.selected = nil
    end
    ...
    function widg:check_key(key, mod)
        if self.items[key] then
            if self.selected then
                self.items[self.selected].background:set_bg(style.color.bg)
            end
            ...
            if self.selected == key then
                ...
                for _, v in ipairs(mod) do
                    modcheck = modcheck and
                                   awful.util.table.hasitem(qlaunch.forcemod, v)
                end
...
function qlaunch:init(args, style)
    ...
    self.keygrabber = function(mod, key, event)
        ...
        for _, k in ipairs(self.keys.all) do
            if redutil.key.match_grabber(k, mod, key) then
                k[3]();
                return
            end
        end
...
function qlaunch:show()
    ...
    redtip:set_pack("Quick launch", self.tip, self.style.keytip.column,
                    self.style.keytip.geometry,
...
function qlaunch:run_or_raise(key, forced_run)
    ...
    if cnum == 0 or forced_run then
        ...
        if self.store[key].run ~= "" then
            awful.spawn.with_shell(self.store[key].run)
        end
    elseif cnum == 1 then
        -- switch to sole app
...
function qlaunch:set_new_app(key, c)
    ...
    if c then
        local run_command = redutil.read.output(string.format(
                                                    "tr '\\0' ' ' < /proc/%s/cmdline",
                                                    c.pid))
        self.store[key] = {app = c.class:lower(), run = run_command, icon = ""}
        local note = redutil.table.merge({
    ...
    if is_file_exists(self.style.configfile) then
        for line in io.lines(self.style.configfile) do
            ...
            if string.find(line, "icon=.*;") then
                local key, app, run, icon = string.match(line,
                                                         "key=(.+);app=(.*);run=(.*);icon=(.*);")
                self.store[key] = {app = app, run = run, icon = icon}
            else
                local key, app, run = string.match(line,
                                                   "key=(.*);app=(.*);run=(.*);")
...
function qlaunch:set_keys(keys, layout)
    ...
    if keys then
        ...
        if layout ~= "all" then
            self.keys.all = awful.util.table.join({}, self.keys.action)
        end
    end
