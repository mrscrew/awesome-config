...
function pulse:change_volume(args)
    ...
    -- get current volume
    local v = redutil.read.output(string.format("pactl get-sink-volume %s", self._sink))
    local parsed = string.match(v, "[Volume: front-left: ](%d+)[ /]")
    -- catch possible problems with pacmd output
    if not parsed then
        naughty.notify({
            title = "Внимание!",
            text = "PA виджет не может проанализировать вывод pactl"
        })
        return
    end
    ...
    -- set new volume
    awful.spawn(string.format("pactl set-sink-volume %s %d", self._sink, new_volume))
...
function pulse:mute()
    ...
    local mute = redutil.read.output(string.format("pactl get-sink-mute %s", self._sink))

    if string.find(mute, "no") or string.find(mute, "нет") then
        awful.spawn(string.format("pactl set-sink-mute %s %d", self._sink, 1))
    else
        awful.spawn(string.format("pactl set-sink-mute %s %d", self._sink, 0))
    end
...
function pulse:update_volume(args)
    ...
    local v = redutil.read.output(string.format("pactl get-sink-volume %s", self._sink))
    local m = redutil.read.output(string.format("pactl get-sink-mute %s", self._sink))

    if v then
        local pv = string.match(v, "[Volume: front-left: ](%d+)[ /]")
        if pv then volume = math.floor(tonumber(pv) * 100 / volmax + 0.5) end
    end

    local mute = not (m and (string.find(m, "no") or string.find(m, "нет")))
...
function pulse.new(args, style)
    ...
    if not widg._sink then
        local st = gears.timer({
            timeout = 1
        })
        local counter = 0
        st:connect_signal("timeout", function()
            counter = counter + 1
            widg._sink = get_default_sink({
                type = widg._type
            })
...
-----------------------------------------------------------------------------------------------------------------------
function pulse.mt:__call(...) return pulse.new(...) end
